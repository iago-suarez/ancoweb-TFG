...
function Detection(videoDetections, id, firstFrame, lastFrame, maxSize, xmlTrajectory) {
    ...
    /**
     * Parse the xml trajectory to a map
     *
     * @private
     */
    this._loadTrajectory = function () {
        this.trajectory.points = {};

        var xmlPoints = $(this.xmlTrajectory).find('point');

        var tmpDet = this;
        $(xmlPoints).each(function () {
            tmpDet.trajectory.points[parseInt($(this).attr('frame'))] =
            {x: parseInt($(this).attr('x')), y: parseInt($(this).attr('y'))};
        });
        this.trajectory.firstFrame = parseInt($(xmlPoints[0]).attr('frame'));
        this.trajectory.lastFrame = parseInt($(xmlPoints[xmlPoints.length - 1]).attr('frame'));
    };
    this._loadTrajectory();
}
...
Detection.prototype.getPositionPoint = function (nFrame) {
    ...
    if (Object.keys(this.trajectory.points).length === 0) {
        return;
    }

    // if the object is not detected on nFrame
    if (!nFrame.between(this.trajectory.firstFrame, this.trajectory.lastFrame)) {
        return;
    }

    if (this.trajectory.points[nFrame] === undefined) {
        //If we have't got a point for this frame we calc it.
        var p1, f1 = nFrame;
        do {
            f1--;
            p1 = this.trajectory.points[f1];
        } while (p1 === undefined);
        var p2, f2 = nFrame;
        do {
            f2++;
            p2 = this.trajectory.points[f2];
        } while (p2 === undefined);
        ...
        this.trajectory.points[nFrame] = {x: Math.round(x), y: Math.round(y)};
    }
    return this.trajectory.points[nFrame];
